...
DatasetFn = Callable[Ellipsis, Mapping[str, mytypes.Dataset]]
...
class DatasetRegistry:
    ...
    def load(self, name, *, batch_size, **kwargs):
...
def load(name, *, batch_size, split=None, preprocessors=None, prefetch=True, **kwargs):
    """Load a dataset registered with the D3PM dataset registry.
...
class TFTextVocabulary:
    ...
    def encode(self, text, shape=None):
        ...
        tokens = mask * tf.constant(value=26, shape=tokens.shape) + (1 - mask) * tokens
        return tokens
    ...
    def decode(self, tokens):
        ...
        tokens = (
            extra_mask * tf.constant(45, shape=tokens.shape) + (1 - extra_mask) * tokens
        )
...
def load_lm1b(
    ...
    for split, ds in datasets.items():
        ...
        ds = ds.padded_batch(
            batch_size, padded_shapes=max_length, padding_values=0, drop_remainder=True
        )
    ...
    if not tf.io.gfile.exists(os.path.join(data_dir, "text8.train.txt")):
        ...
        for split, data in splits.items():
            with tf.io.gfile.GFile(
                os.path.join(DATA_DIR, "text8." + split + ".txt"), "w"
            ) as f:
                f.write(data)
