...
class CachedMatrixPowerState:
    ...
    @staticmethod
    def precompute(
        matrix, max_power=100, precision=jax.lax.Precision.DEFAULT, use_numpy=False
    ):
    ...
    def matrix_power_multiply(
        self, x, power, transpose=False, precision=jax.lax.Precision.DEFAULT
    ):
        ...
        def power_body_fn(state, arr):
            ...
            new_power = jnp.where(
                cond, state.current_power - power, state.current_power
            )
        ...
        init_state = PowerState(step=num_steps - 1, current_power=power, current_x=x)
        ...
        return jnp.where(
            final_state.current_power == 0,
            final_state.current_x,
            jnp.full_like(final_state.current_x, jnp.nan),
        )

    def matrix_power(self, power, precision=jax.lax.Precision.DEFAULT):
...
class LazyMatrixPowerState:
    ...
    def matrix_power_multiply(
        self, x, power, transpose=False, precision=jax.lax.Precision.HIGHEST
    ):
        """Computes matrix vector product jnp.linalg.matrix_power(M, power) @ x.
    ...
    def matrix_power(self, power, precision=jax.lax.Precision.HIGHEST):
        """Computes matrix power jnp.linalg.matrix_power(M, power) efficiently.
...
def get_nearest_neighbors(
    embeddings, num_chunks=16, k=10, return_distances=False, include_self=False
):
    ...
    def nn_body_fn(start_idx, _):
        embed_slice = jax.lax.dynamic_slice(
            embeddings, (start_idx, 0), (interval, embeddings.shape[1])
        )[:, None]
...
def transition_rate_expm(matrix, target_diagonal=1e-3, renormalize_cols=True):
    ...
    iterations_for_diagonal = jnp.ceil(jnp.log2(max_diag) - jnp.log2(target_diagonal))
    ...
    iterations = jnp.maximum(iterations_for_diagonal, iterations_for_mixing).astype(
        jnp.int32
    )
...
def partition_embeddings_hierarchically(embedding_matrix, special_tokens_at_front=0):
...
def _partition_embeddings_hierarchically(
    embedding_matrix, indices, force_keep_at_front=0
):
    ...
    projected = (
        sklearn.decomposition.PCA(1).fit_transform(relevant_embeddings).squeeze(1)
    )
...
def compute_relative_information_removal(
    transition_matrix, initial_distribution, use_perplexity=False
):
    ...
    log_transition = log_transition - jax.scipy.special.logsumexp(
        log_transition, axis=0, keepdims=True
    )
    log_initial = jnp.log(initial_distribution)
    log_initial = log_initial - jax.scipy.special.logsumexp(
        log_initial, axis=0, keepdims=True
    )
    ...
    joint_entropy = -jnp.sum(
        jnp.where(log_joint == -np.inf, 0.0, jnp.exp(log_joint) * log_joint)
    )
    initial_entropy = -jnp.sum(
        jnp.where(log_initial == -np.inf, 0.0, jnp.exp(log_initial) * log_initial)
    )
    marginal_after_entropy = -jnp.sum(
        jnp.where(
            log_marginal_after == -np.inf,
            0.0,
...
def compute_information_removal_samples_closed_form(
    ...
    """
    query_exponents = jnp.geomspace(min_exponent, max_exponent, interpolation_steps)
...
@gin.configurable
def compute_information_removal_samples_by_squaring(
    rate_matrix,
    initial_distribution,
    min_exponent=1e-4,
    max_exponent=1e5,
    ...
    powers_of_two = 1 + jnp.ceil(
        jnp.log2(max_exponent) - jnp.log2(min_exponent)
    ).astype(jnp.int32)
...
@gin.configurable
def build_mutual_information_schedule(
    schedule_steps,
    exponents,
        ...
        if allow_out_of_bounds:
            if allow_out_of_bounds == "warn":
                logging.warning(
                    ...
                    schedule_info_removals[-1],
                    information_removals[-1],
                )
            schedule_info_removals = np.linspace(
                0, information_removals[-1], schedule_steps + 1
            )[1:]
        else:
            raise ValueError(
