...
import diffusion  # pylint: disable=unused-import
import models
import tasks
import trainers
import utils
...
flags.DEFINE_string(
    "ckpt_dir", None, "Directory for storing experiment info and log files."
)
...
def run_experiment(
    ...
    if jax.process_index() == 0:
        _write_gin_configs(os.path.join(model_dir, "all_config.gin"), operative=False)
    ...
    ds = datasets.load(
        name=dataset_name, batch_size=batch_size, preprocessors=task.preprocessors
    )
    ...
    if "valid" not in ds:
        ...
        assert (
            "test" in ds
        ), f"dataset {dataset_name} must contain a test or validation set."
    ...
    for step, batch in zip(range(start_step, max_train_steps), train_iter):
        if step % validate_every == 0:
            ...
            if jax.process_index() == 0:
                ...
                for metric in prediction_summary:
                    logging.info(
                        "eval/%s @ step %d: %s", metric.name, step, str(metric.value)
                    )
            ...
            if jax.process_index() == 0:
                logging.info(
                    "eval/eval_time: %f (@ step %d)", eval_tock - eval_tick, step
                )
        ...
        if jax.process_index() == 0 and (
            (checkpoint_frequency and step % checkpoint_frequency == 0 and step > 0)
                ...
                for key, val in summary.items():
                    if jnp.isnan(val):
                        if fail_on_nan:
                            ...
                            logging.warning(
                                "Encountered NaN in %s at step %d.", key, step
                            )
