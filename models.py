...
class TimestepEmbedBlock(nn.Module):
    ...
    def __call__(self, x, timestep):
        logging.info(
            ...
            self.num_steps,
            self.use_sinusoidal_embeddings,
        )
...
class EncoderDecoder(nn.Module, param_remapping.ParameterRemappable):
    ...
    def setup(self):
        self.token_embedder = (
            ...
            if self.shared_token_embedder_factory
            else None
        )
        self.encoder = self.encoder_factory(shared_token_embedder=self.token_embedder)
        self.decoder = self.decoder_factory(shared_token_embedder=self.token_embedder)

    def encode(
        self, encoder_input_tokens, encoder_padding_mask, *, deterministic=False
    ):
        ...
        return self.encoder(  # pytype: disable=attribute-error
            encoder_input_tokens, encoder_mask=encoder_mask, deterministic=deterministic
        )
...
class DecoderOnly(nn.Module, param_remapping.ParameterRemappable):
    ...
    def __call__(
        self, decoder_input_tokens, decoder_padding_mask, *, deterministic=False
    ):
        ...
        *,
        ...
        if self.parallel:
            ...
            if timestep is not None and self.use_film_layers:
                logging.info(
                    "Using FiLM decoder block with num_steps: %d.", self.num_steps
                )
            ...
            y = self.self_attention(
                x, x, decoder_mask, decoder_bias, decode=decode
            ) + self.mlp(x, decode=decode)
            if encoded is not None:
                y += self.encoder_decoder_attention(
                    x, encoded, encoder_decoder_mask, encoder_decoder_bias
                )
            ...
            if encoded is None:
                ...
                if self.encoder_decoder_attention is None:
                    raise ValueError(
                        "Expected encoder_decoder_attention to be populated "
                ...
                y = self.encoder_decoder_attention(
                    y, encoded, encoder_decoder_mask, encoder_decoder_bias
                )
        ...
        z = activation_partitioning.with_sharding(z, self.activation_partitioning_dims)
...
class Decoder(nn.Module, param_remapping.ParameterRemappable):
    ...
    def setup(self):
        ...
        if (self.token_embedder_factory, self.shared_token_embedder).count(None) != 1:
            raise ValueError(
...
class ModelFactory:
    ...
    def _shared_relative_position_bias(self):
        return (
            self.config.position_embeddings == "relative"
            and not self.config.relative_attention_per_layer
        )
    ...
    def _per_layer_relative_position_bias(self):
        return (
            self.config.position_embeddings == "relative"
            and self.config.relative_attention_per_layer
        )
    ...
    def _make_final_layer_norm(self):
        return layer_norm.T5LayerNorm(use_scale=self.config.final_layer_norm_use_scale)
    ...
    def _make_encoder_layer(self, *, shared_relative_position_bias):
        return t5_architecture.EncoderLayer(
            ...
            relative_position_bias_factory=(
                ...
                if self._per_layer_relative_position_bias
                else None
            ),
        ...
        *,
        ...
        return t5_architecture.Encoder(
            ...
            token_embedder_factory=(
                ...
                else self._make_encoder_token_embedder
            ),
            position_embedder_factory=(
                self._make_encoder_position_embedder if self._abs_posemb else None
            ),
            shared_relative_position_bias_factory=(
                self._make_relative_position_bias
                if self._shared_relative_position_bias
                else None
            ),
        ...
        *,
        ...
        return Decoder(
            ...
            token_embedder_factory=(
                None
                if self.config.share_embeddings
                else self._make_decoder_token_embedder
            ),
            position_embedder_factory=(
                self._make_decoder_position_embedder if self._abs_posemb else None
            ),
            shared_relative_position_bias_factory=(
                self._make_relative_position_bias
                if self._shared_relative_position_bias
            ...
            output_logits_factory=(
                self._make_decoder_output_logits
    ...
    def make_encoder_decoder(self):
        return EncoderDecoder(
            shared_token_embedder_factory=(
                self._make_shared_token_embedder
                if self.config.share_embeddings
                else None
            ),
...
def make_encoder_decoder(
    config, num_steps=1000, use_timestep_embeddings=False, use_film_layers=False
):
    return ModelFactory(
        config=config,
        num_steps=num_steps,
        use_timestep_embeddings=use_timestep_embeddings,
        use_film_layers=use_film_layers,
...
class CategoricalDiffusionModel(nn.Module):
    ...
    def setup(self):
        ...
        logging.log_first_n(logging.INFO, "CategoricalDiffusionModel configuration:", 1)
    ...
    def encode(self, encoder_input_tokens, encoder_padding_mask):
        ...
        (encoder_input_tokens, encoder_padding_mask) = expand_dims(
            encoder_input_tokens, encoder_padding_mask
        )
