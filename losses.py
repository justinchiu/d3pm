...
def _cross_entropy_with_logits(logits, targets):
    ...
    assert logits.shape == targets.shape, (
        "logits and targets must have the same"
...
_cross_entropy_with_logits.defvjp(
...
def cross_entropy_with_logits(logits, targets, label_smoothing=0.0):
...
def cross_entropy_with_probs(probs, targets, label_smoothing=0.0, epsilon=1e-20):
...
def kl_divergence_with_logits(p_logits=None, q_logits=None, temperature=1.0):
...
def lp_norm(inputs, targets, p=2, apply_root=False):
    ...
    assert inputs.shape == targets.shape, (
        f"Inputs and target shapes must agree."
...
def cosine_distance(inputs, targets, epsilon=1e-20):
    ...
    loss = 1 - (inputs * targets).sum(axis=-1) / (inputs_norm * targets_norm + epsilon)

    return loss


def weighted_accuracy(logits, targets, weights=None):
...
def weighted_mean(array, weights=None):
...
def classification_loss_fn(
    params, inputs, targets, *, model_apply, rng_key, is_eval=False, label_smoothing=0.0
):
    ...
    if isinstance(output, tuple):
        if len(output) != 2:
            raise ValueError(
                "expected model_apply to return logits or (logits, mask)"
    ...
    loss = cross_entropy_with_logits(logits, targets, label_smoothing=label_smoothing)
