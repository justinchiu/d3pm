...
def _pmap_preprocess_batch(batch, features=None, disabled=False, max_batch_size=None):
    ...
    def reshape_arr(arr):
        reshaped = arr.reshape(
            (jax.local_device_count(), arr.shape[0] // jax.local_device_count())
            + arr.shape[1:]
        )
...
def build_vmapped_loss(
    ...
    *,
    ...
    def full_loss_fn(params):
        ...
        (loss, denominator), (metrics, extras) = batch_loss_fn(params, batch, rng_key)
...
def standard_train_step(
    ...
    *,
    ...
    grads_ok = jnp.all(
        jnp.asarray([jnp.all(jnp.isfinite(p)) for p in jax.tree_leaves(new_optimizer)])
    )
...
class Trainer:
    ...
    def __init__(
        ...
        **train_kwargs,
        ...
        self.aux_state = self.task.state_init_fn(dataset_info=dataset_info, task=task)
    ...
    def update_aux_state(self):
        ...
        if self.state_update_fn and (
            step == 0 or step % self.aux_state.dynamic_update_freq == 0
        ):
            new_dynamic_state = self.state_update_fn(
                self.aux_state.dynamic_state, self.state.optimizer.target
            )
    ...
    def evaluate_batch(self, batch):
        ...
        metrics, extras, self.model_rng = self.eval_fn(
            self.state.optimizer, batch, self.model_rng, self.aux_state.dynamic_state
        )
    ...
    def predict_batch(self, batch, **kwargs):
        ...
        predictions, self.model_rng = predict_fn(
    ...
    def num_params(self, unreplicate=True):
        ...
        return utils.num_params(self.state.optimizer.target, unreplicate=unreplicate)
    ...
    def load_checkpoint(self, ckpt_dir, **kwargs):
        ...
        if state is not restored_state:
            ...
            logging.info(
                "No checkpoint found in directory %s. Using new params.", ckpt_dir
            )
