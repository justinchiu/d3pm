...
def build_dataset(ds, vocab=None):
    ...
    Returns:
      a mytypes.Dataset dataset object.
    """
    if ds is None:
        return None

    return mytypes.Dataset(
        dataset=ds,
        _vocab=vocab,
    )


def wrap_datasets(train=None, valid=None, test=None, *, vocab=None):
    """Create a D3PM Dataset object.
    ...
    return {
        k: build_dataset(v, vocab=vocab) for k, v in datasets.items() if v is not None
    }


get_dataset_info = mytypes.get_dataset_info


def get_dataset_info_from_batch(batch, *, vocab=None):
    """Wraps a set of TFDS datasets with vocabularies."""
    shapes = jax.eval_shape(lambda: batch)

    dataset_info = mytypes.DatasetInfo(features=batch.keys(), vocab=vocab, shapes=shapes)

    return dataset_info


def build_batch_from_info(dataset_info, prune_batch_dim=True):
...
def make_model_apply(model, rng_key):
    ...
    def model_apply(params, *args, method="__call__", allow_missing=False, **kwargs):
...
def combine_metrics(step_metrics):
    ...
    metrics_all = jax.tree_map(lambda x: jnp.asarray(x, dtype=np.float32), metrics_all)
        ...
        for name in factors:
            if name == "constant":
                ...
                interpolation = jnp.minimum(
                    1.0, interpolation
                )  # now 1 until warmup_steps, then linear decay to 0
...
def predict(model, eval_ds, metric_fns, dataset_info, num_predict_steps=None):
    ...
    Args:
      ...
      dataset_info: a mytypes.DatasetInfo object.
        ...
        def set_path(d, path, value):
            ...
            if k in d:
                if value.shape != d[k].shape:
                    raise ValueError(
                        "Shape mismatch: %s" % str((k, value.shape, d[k].shape))
                    )
            d[k] = value
            return d
            ...
            if isinstance(x, jax.core.Tracer):
                ...
                if short:
                    ...
                    if np.issubdtype(x.dtype, np.floating):
                        info += (
                            f" {np.mean(flat):.2} ±{np.std(flat):.2} "
